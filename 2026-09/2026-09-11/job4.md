...
## 追加（第七轮）：不灭残魂（passive_000037）还有哪些怪物在用（只读）
...
**有，共 6 只其他怪物**（含五层 Boss 共 **7 只**）。全项目扫描：

- `passive_000037` 仅被 **7 个 monster 配置**引用（`assets/config` 内共 9 处，另 2 处是 `skill_passive.json` 自身定义）；
- `effect_100089`（不灭残魂效果本体）在配置层**只**被 `passive_000037` 引用 → **不存在第二个被动/道具/道纹/法宝绕过它给这个效果**；代码里也无硬编码注入（`has_effect("effect_100089")` 只是判定）。

### 清单

| # | 怪物 | 等级/类型 | 品阶 | 气血 | 同带其他被动 | 出场战斗 |
|---|------|-----------|------|------|--------------|----------|
| 1 | `monster_900025` 上古剑灵残魂 | 70 / boss | epic | 49932 | — | `combat_106002`（镇妖塔五层） |
| 2 | `monster_900028` 法宝器灵·镇海 | 70 / boss | epic | 48000 | `passive_000054` 巨量回血 | `combat_108002`（npc 触发） |
| 3 | `monster_008010` 金甲禁卫 | 90 / 普通怪 | normal | 53760 | `passive_000047` 战神降临 | `combat_108005`（东海·法宝大殿） |
| 4 | `monster_930603` 镇谷器灵 | 78 / boss | epic | 58700 | `passive_000054` 巨量回血 | `tower_f02_03`（万妖谷·第六层） |
| 5 | `monster_931005` 巫祭长老 | 90 / 普通怪 | normal | 54500 | `passive_000047` 战神降临 | `tower_f04_01`（万妖谷·第十层） |
| 6 | `monster_931203` 破军妖将 | 98 / 普通怪 | normal | 53000 | `passive_000047` 战神降临 | `tower_f04_03`（万妖谷·第十二层） |
| 7 | `monster_900040` 凶兽·蜚（残躯） | 80 / boss | epic | 65000 | — | **无（孤儿配置）** |

### 三种带法（同一被动、不同搭配）

| 搭配 | 怪物 | 实战含义 |
|------|------|----------|
| 单带不灭残魂 | 上古剑灵残魂、凶兽·蜚 | 纯“多一条命” |
| 不灭残魂 + 战神降临（`effect_100103`：复活时额外回 30% 血、+30% 攻速、+30% 物伤、+20% 吸血，每级 +1%） | 金甲禁卫、巫祭长老、破军妖将 | 复活后直接进入强化态，“越打越猛” |
| 不灭残魂 + 巨量回血（`effect_100111`：每秒回 10% 最大生命） | 法宝器灵·镇海、镇谷器灵 | 持续回血 + 硬复活，极为坦克，建议优先爆伐/禁疗 |

### 两点发现（待拍板，本次未改）

| # | 发现 | 说明 |
|---|------|------|
| 1 | `monster_900040`「凶兽·蜚（残躯）」Lv80 epic Boss **未被任何战斗引用**（全 `assets/config` 内只在自身文件出现） | 孤儿配置：要么是未上线内容，要么漏挂战斗；其他 6 只均有出场 |
| 2 | 7 只全部不在裂隙副本路径（裂隙战斗 `rift_f0x_*` 不含它们） | 因此实战均为技能 lv1 = **20% 血复活**；若将来被裂隙引用，`enhance_monster_skills_for_rift` 会 +8 级拉满（lv13 → **56% 血**）——数值会跳变，需提前评估 |

### 证据链

| # | 层面 | 证据 |
|---|------|------|
| 1 | 配置全扫 | `rg "passive_000037" assets/config` → 7 个 `monster/*.json` + `skill_passive.json` 定义；`rg "effect_100089" assets/config` → 仅 `skill_passive.json`（+ `attack_effect.json` 自身定义） |
| 2 | 出场校验 | `combat.json` 中逐条匹配 `characters[].monster_id`（脚本枚举），得 `combat_106002`、`combat_108002`、`combat_108005`、`tower_f02_03`、`tower_f04_01`、`tower_f04_03`；`900040` 无命中 |
| 3 | 技能等级 | `game_world.gd:2683` 仅裂隙置 `should_enhance_monster_skills=true`；`combat_init_tool.enhance_monster_skills_for_rift()` 为 `increase_max_level(8)` + `level_up_to_max()` |
| 4 | 搭配被动数值 | `attack_effect.json`：`effect_100103=[0.3,0.3,0.3,0.2]`（战神降临）、`effect_100111=[1,0.1,-1,1]`（巨量回血，每秒 10% 最大生命） |
| 5 | 自动化已有用例 | `src/test/combat/test_combat_auto.gd` 已覆盖上古剑灵残魂（381）、金甲禁卫（707）、法宝器灵·镇海（1026）三例复活断言 |

> 本轮仅只读核实，未改动任何配置或代码；扫描明细见 `./file/bumie-canhun-monsters.txt`。
...
| 2026-09-11 18:35 | 第七轮（只读）：不灭残魂（passive_000037）全项目排查——共 7 只怪携带（另 6 只：法宝器灵·镇海、金甲禁卫、镇谷器灵、巫祭长老、破军妖将、凶兽·蜚（残躯））；effect_100089 无其它授予途径；发现 `monster_900040` 为孤儿配置、7 只均不在裂隙路径（实战恒为 lv1=20% 血） |
