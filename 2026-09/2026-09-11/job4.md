...
| 状态 | 已完成（Q1 已确认；Q2 按方案 A 实施并 push：`origin/master` = `04bc7bf0`；真机验收待设备接入） |
| 预估耗时 | 0.3h |
...
- [ ] 真机验收（二层守关 Boss 战：非 4 只幻音妖后；战后对话开门正常）；如需同步设备跑 `bash sync_android.sh`
...
临时探针脚本（不入 git）：`../../../work_record_temp/2026-09/2026-09-11/scripts/check_huanyin_skill.gd`、`check_combat_106009.gd`。
...
- [ ] 真机验收：二层守关 Boss 战是否确为 1 幻音妖后 + 3 魇影（不再 4 只 Boss），战后对话能正常开门并移除 NPC
- [ ] 真机体验后评估难度：3×魇影（梦魇 50% 沉睡×2 目标）+ Boss 光环是否过强
