...
| 状态 | 已完成（Q1 确认；Q2/Q3 已实施并 push：`origin/master` = `d40d4c7d`；五层剑灵战是否同步调整待用户拍板；真机验收待设备） |
| 预估耗时 | 0.3h |
...
| 五层 | `combat_106002` | `npc_104012`（上古剑灵）/`dialogue_106006` | `auto_count` × 4 只剑灵残魂 | **暂未改（待定）**——设计文档定位是「剑术对决」1v1 且为可重复挑战 Boss |
...
3. 落位沿用二层已验收的菱形：Boss `(2,1)` / 左 `(1,2)` / 右 `(3,2)` / 前 `(2,3)`。
4. 全部关闭 `auto_count`（用户明确要求），4 条显式条目，保证与队伍人数解耦且 Boss 必出场（任务开门依赖 `kill_monster`）。

### 验证（2026-09-11 10:5x）

- headless 真实初始化路径（`CombatInitTool.init_combat_with_config`）逐场验证 106008~106011：队伍 1/2/4 人下均固定为 **4 只 = 1 Boss + 3 普通怪**，落位固定，**24 断言全 PASS / 0 FAIL**（输出见 `./file/tower-boss-combats-probe.txt`）。
- `python validate_configs.py`：全量通过（0 错误，7 个既有无关警告）。
- 提交并 push：`04bc7bf0..d40d4c7d`（`origin/master` = `d40d4c7d`）。
...
- [ ] **待用户拍板：五层上古剑灵战（`combat_106002`）是否也改为「剑灵 ×1 + 双首妖狼/噬魂妖将 ×3」**（设计文档写的是 1v1 剑术对决、且为可重复挑战 Boss，暂未改）
...
- 提交并 push：`d40d4c7d`。
- 五层 `combat_106002` **暂未改**：设计文档 `stage7_huashen_quest_structure.md:388-424` 把它定义为「剑术对决」1v1，且 `npc_104012` 是**不删除的可重复挑战 Boss**；且剑灵本身就是输出型（万剑覆雨 + 不死残魂），不存在「辅助怪没人输出」问题。已在回复中确认是否也要改成「剑灵 + 3 普通怪」。
...
| `./file/tower-boss-combats-probe.txt` | headless 探针输出：`combat_106008~106011` 四层守关 Boss 战构成与落位（24 断言 PASS；附五层 `combat_106002` 现状参考行） |
...
- [ ] 真机验收：一/二/三/四层守关 Boss 战均为 1 Boss + 3 普通怪（不再 4 只 Boss），战后对话能正常开门并移除 NPC
- [ ] 待拍板：五层上古剑灵战是否也加 3 只普通怪（涉难度与「剑术对决」叙事）
- [ ] 真机体验后评估难度：二层 3×魇影（梦魇 50% 沉睡×2 目标）/ 四层 3×罗刹（10 次 50% 均摊）是否过强
